#对于关系链接，使用HX的freebase embedding
...
class FBEmbeddingBasedSimilarity:
    def __init__(self, dataset) -> None:
        """
        使用TextAda计算问题和关系embedding，计算相似度获取
        数据集是GrailQA，CWQ，WebQSP
        """
...
class PLMSimRanker:
    """
    使用sentencebert等语义相似度模型，来快速计算相似度
    特点是不用跑embedding，即算即用
    """
...
def calculate_question_rel_topk(dataset, split, output_dir):
    ...
    if dataset == "CWQ":
        ...
        if split == "dev":
            questions = [item['question'] for item in load_json("/home5/yhbao/data/cwq/cwq_dev_linking.json")]
        elif split == "train":
            questions = [item['question'] for item in load_json("/home5/yhbao/data/cwq/cwq_train_linking.json")]
        else:
            questions = [item['question'] for item in load_json("/home5/yhbao/data/cwq/cwq_test_linking.json")]
