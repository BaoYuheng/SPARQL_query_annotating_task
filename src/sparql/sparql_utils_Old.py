...
REVERSE_RELATION_PATH = "/home5/yhbao/data/input/common/freebase_reverse_relation_map.json"
RELATION_DOMAIN_RANGE_PATH = "/home5/yhbao/data/input/common/fb_relations_domain_range_label.json"
...
class TreeConstructor:
    ...
    def _construct_syntax_tree(self, parent_node:GGPTreeNode, current):
        ...
        # Terminal node
            ...
            # TODO: @yhbao, 对于 Wikidata 上的数据集，需要实现
    ...
    def _reorder_children(self, current_node:GGPTreeNode):
        ...
        #目前只考虑GGP与UnionGP内元素的重排序。
        for child in current_node.children:
            self._reorder_children(child)
        if current_node.type == GGP or current_node.type == UnionGP or current_node.type == BGP:
            if len(current_node.children) > 1:
                #current_node.children.sort(key = lambda i: mask_var_index(i))
    ...
    def canonicalize_syntax_tree(self, root_node:GGPTreeNode):
        ...
        if self.dataset_type in [
            DATASET.CWQ, DATASET.GRAIL, DATASET.WEBQ, DATASET.SIMULATED_FREEBASE
...
class TreeEditDistance:
    ...
    def get_label(node:GGPTreeNode):
        # TODO:@yhbao GGPTreeNode 中，标记变量节点
        if isinstance(type(node.node), RDFTerm):
            if not node.node.is_constant:
                return TreeEditDistance.VAR_TAG # 假设变量的 label 都是一样的
        return node.to_str()
...
class SyntaxTreeEditor:
    ...
    def remove_outer_select_grailqa(self):
