from src.sparql.awudima import parser
from src.sparql.awudima import (
...
from src.core.common import DATASET, Dataset, Method, KB_TYPE, FB_DATASETS, FB_METHODS, WD_METHODS, to_dataset, to_method, to_kb_type
...
# 原为 whzhou 机器上的绝对路径，现拷贝到本仓库 data/ 目录下。
# Originally hardcoded absolute paths on whzhou's machine; copied to the repo's data/ directory.
# 2026-01-22 ESWC rebuttal
...
REVERSE_RELATION_PATH = "data/FB_ontology/freebase_reverse_relation_map.json"
RELATION_DOMAIN_RANGE_PATH = "data/FB_ontology/fb_relations_domain_range_label.json"
WIKIDATA_REVERSE_RELATION_PATH = "data/WD_ontology/reverse_properties_wikidata"
...
class TreeConstructor:
    ...
    def _add_reverse_relations(self, root_node:GGPTreeNode):
        ...
        kb = to_kb_type(self.dataset_type)
    ...
    def canonicalize_syntax_tree(self, root_node:GGPTreeNode):
        ...
        if to_kb_type(self.dataset_type) == KB_TYPE.FREEBASE:
            # 仅对于 Freebase 上的数据集/方法，考虑类型约束
            self._add_domain_range(root_node)
...
class SyntaxTreeEditor:
    ...
    def _traverse_leaf_nodes(self, current):
        ...
        # Terminal node
            ...
            self.leafs.append(current)
            # 2026-06-02 Yuheng Bao: Extract individual entities from VALUES
            # clause so they can be found by update_leaf_value /
            # replace_leaf_with_variable.  Without this, queries like
            # VALUES ?x { wd:Q867257 wd:Q870134 } cannot have their grounded
            # entities abstracted into replace variables.
            for v_list in current.values:
                for v in v_list:
                    if isinstance(v, RDFTerm):
                        self.leafs.append(v)
                        if "?" not in v.to_str() and "P" not in v.to_str():
                            self.items.add(v)
            return
    ...
    def _remove_extra_filter_lines(self):
        ...
        if self.dataset in [DATASET.CWQ, DATASET.WEBQ]:
            ...
            filtered_lines = [
                ln for ln in lines
                if not (
                    (ln.startswith('FILTER') and (' != ' in ln)) or
    ...
    def _break_wdt_wikidata2(self):
        ...
        for bgp in bgps:
            ...
            for tp in bgp.triples:
                if isinstance(tp.property, RDFTerm) and tp.property.value.startswith("http://www.wikidata.org/prop/direct"):  # expanded IRI form
                    pid = tp.property.value.split("/")[-1]  # extract Pxxx from expanded IRI
    ...
    def preprocess_edit_distance(self, target_variable_name):
        ...
        if self.dataset in [DATASET.CWQ, DATASET.WEBQ, DATASET.QGG, DATASET.QUERYAGENT, DATASET.BINDER]:
            self._update_time_literal_format()
        if to_kb_type(self.dataset) == KB_TYPE.WIKIDATA:
            # 2026-06-03 Yuheng Bao: Reverted from _break_wdt_wikidata2 back to _break_wdt_wikidata.
            # Reasoning: _break_wdt_wikidata converts wdt:P to PropertyPath p:P/ps:P.
            # Both golden (already PropertyPath) and constructed (converted) end up with the same
            # representation, so construct_syntax_tree decomposes both with ?exist_N variables.
            # The order-dependent ?exist_N numbering is then normalized by canonicalize_syntax_tree
            # → _rename_vars (sorts triples by constants, renames all vars to $var0, $var1, ...).
            # This is the original STAR-QC approach but WITH canonicalization enabled (unlike original).
            self._break_wdt_wikidata()
        # 2026-06-03 Yuheng Bao: Added SIMULATED_WIKIDATA so constructed queries
        # (dataset=SIMULATED_WIKIDATA) get DISTINCT too, matching golden (dataset=LC2).
        # Without this, golden gets DISTINCT but constructed doesn't → spurious TED difference.
        # Both REQUMA (our dataset) and LC-QuAD 2.0 target Wikidata KG.
        if self.dataset in [DATASET.LC2, DATASET.QALD, DATASET.QGG, DATASET.SIMULATED_WIKIDATA]:
