"""
来源: (Sophia) /home/yhbao/question_analyze_and_construction/awudima/sparql/parser.py
"""
# -*- coding: utf-8 -*-
...
from . import RDFTerm, Expression, PathTerm, \
...
def t_ID(t):
    ...
    # '[a-zA-Z_][a-zA-Z_0-9\-]*'
    lower_list = ['bnode']
    if t.value not in lower_list:
        t.type = reserved.get(t.value.upper(), 'ID')  # Check for reserved words   #666666
...
def p_describe_query_0(p):
    ...
    iris = [RDFTerm(p[2], is_const=False)] + p[3]
...
def p_arg_list_1(p): #666
    """
    arg_list :  LPAR distinct expression more_args RPAR 
             |  LPAR expression more_args RPAR
    """
    ...
    #首先，判断是否有distinct
    if len(p) == 6:
        p[0] = Expression([p[3]]+p[4], [[2]])   #distinct(a,b...)
    else:
        if len(p[4]) == 0:
            p[0] = p[1]     #直接把唯一的参数向上传递
        else:
            args = [p[2]] + p[3]    #返回一个参数列表
            p[0] = args
...
def p_triples_same_subject_path_0(p):
    ...
    p[0] = []
    blank_node_added = False
    for pr, o in p[2]:
        #特殊判断[]
        if isinstance(o, tuple) and o[0].is_bnode:
            if not blank_node_added:
                blank_node_added = True
                p[0].append(TriplePattern(p[1], pr, o[0]))
            p[0].append(TriplePattern(o[0], o[1], o[2]))
        else:
            p[0].append(TriplePattern(p[1], pr, o))
...
def p_triples_same_subject_path_1(p):
    ...
    #取p[0]第一个的subject作为p2的subject
    p[0] = [TriplePattern(s, pr, o) for s, pr, o in p[1]]
    former_head = p[0][0].subject
    if former_head.is_bnode:
        p[0] += [TriplePattern(former_head, pr, o) for pr, o in p[2]]
    else:
        p[0] += p[2]
...
def p_path_primary_2(p):
    """
    path_primary : NEG path
    """
    p[0] = '!' + str(p[2])

def p_path_primary_3(p):
    """
    path_primary : NEG bracketed_path
    """
    p[0] = '!' + str(p[2])


def p_path_primary_4(p):
    """
...
def p_conditional_or_expression(p):     #666
    """
    conditional_or_expression : conditional_and_expression ORSYMB conditional_or_expression
...
def p_conditional_and_expression(p):    #666
    """
    conditional_and_expression : value_logical ANDSYMB conditional_and_expression
                               | value_logical
    """
    if len(p) > 2:
    #if p[2] is not None:
...
# Helpers
xstring = ""
# idx for allocate blank nodes
blank_node_idx = 0 
_lexer = lex.lex(debug=1)
_sparql_parser = yacc.yacc(debug=1)
_filter_parser = yacc.yacc(start = "filter")
_bgp_parser = yacc.yacc(start = "triples_block")

def parse_sparql(string):
    global xstring
    global blank_node_idx
    blank_node_idx = 0   #reset blank node idx
    xstring = string
...
def parse_filter(string):
    ...
    blank_node_idx = 0   #reset blank node idx
    xstring = string
    return _filter_parser.parse(string, lexer=_lexer)


def parse_bgp(string):
    global xstring
    global blank_node_idx 
    blank_node_idx = 0   #reset blank node idx
    xstring = string
    return _bgp_parser.parse(string, lexer=_lexer)
