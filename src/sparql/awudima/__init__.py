...
class SelectQuery(Query):
    ...
    def to_str(self):
        ...
        rep += f"\n {super().modifiers_str()}" # 如果没有这一行的话，树结构里面不含 order by 等信息
...
class ValuesClause:
    def __init__(self, variables, values):
        ...
        self.values = values # nested list
    ...
    def expand_syntax_forms(self, prefixes):
        for v_list in self.values:
            for v in v_list:
                v.expand_syntax_forms(prefixes)
...
class BGP:
    ...
    def to_str(self):
        ...
        if not triple_str.strip().endswith('.'):
            triple_str = f"{triple_str} . "
...
class RDFTerm:
    ...
    def expand_syntax_forms(self, prefixes):
        ...
        # 2026-06-02 Yuheng Bao: Only expand prefixed IRIs, not typed/string
        # literals.  Typed literals like "2017-01-01T00:00:00Z"^^xsd:dateTime
        # can contain colons in their value — the original `split(':')` would
        # produce more than 2 parts and raise "too many values to unpack".
        if self.is_constant and not self.is_bnode and self.is_iri:
            if self.value == 'a':
                self.value = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type'

            if ':' in self.value:
                prefix, val = self.value.split(':', 1)
...
class Expression:
    ...
    def to_str(self):
        ...
        if self.oper.upper() in unary_operators or self.oper in unary_operators:
            if isinstance(self.left_expr, list) and len(self.left_expr) == 1:
                return self.oper + "(" + str(self.left_expr[0]) + ")"
            else:
                return self.oper + "(" + str(self.left_expr) + ")"
        elif self.oper.upper() in unary_expression_list:            #666这里的CONCAT是有问题的
            # return '(' + str(self.left_expr) + ' ' \
            #     + self.oper + " ( expr_list[" + ", ".join([str(v) for v in self.right_expr]) + "] ))"
            return '(' + str(self.left_expr) + ' ' \
                + self.oper + " ( " + ", ".join([str(v) for v in self.right_expr]) + "))"
        elif self.oper.upper() in unary_expression_list_without_right:            #666这里的CONCAT是有问题的
            return '(' \
                   + self.oper + " (" + ", ".join([str(v) for v in self.left_expr]) + "))"
            ...
            if self.quaternary_expr is not None:            #有4个参数
                return self.oper + "(" + str(self.left_expr) + "," + str(self.right_expr) \
            ...
            if self.ternary_expr is not None:
                ...
                if isinstance(self.left_expr, GGP):
                    return "(" + self.oper + " " + "{ \n" +str(self.left_expr) + "})"
                return "(" + self.oper + " " + str(self.left_expr) + ")"
    ...
    def expand_syntax_forms(self, prefixes):
        if isinstance(self.left_expr, list):    #bao:如果是一个arglist
            for arg in self.left_expr:
                arg.expand_syntax_forms(prefixes)
        else:
...
unary_expression_list = {           #666 CONCAT和COALESCE是错的
    'IN': '',
    'NOT IN': '',
    # 'CONCAT': '',
    # 'COALESCE': ''
...
unary_expression_list_without_right = {
    'CONCAT': '',
    'COALESCE': ''
}
binary_operators = {
